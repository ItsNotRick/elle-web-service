...
class CardImage(Resource):
  ...
  def post(self, _id):
    ...
    if file and allowed_file(file.filename):
      ...
      filename = get_id_file_name(str(_id)) + file_extension #, filename)
      file.save(cross_plat_path('uploads/' + filename))

      query = "INSERT INTO images VALUES (%s, %s)"
      new_path = cross_plat_path('Images/' + filename)
      post_to_db(query, (_id, new_path))

      #os.system('mv ./uploads/' + filename + ' ./Images/' + filename)
      os.rename(cross_plat_path('uploads/' + filename), cross_plat_path('Images/' + filename))
...
class CardSound(Resource):
  ...
  def post(self, _id):
    ...
    if file and allowed_file(file.filename):
      filename, file_extension = os.path.splitext(file.filename)
      filename = get_id_file_name(str(_id))
      file.save(cross_plat_path('uploads/' + filename + file_extension))

      query = "INSERT INTO audio VALUES (%s, %s)"
      new_path = cross_plat_path('Audio/' + filename + '.ogg')
      post_to_db(query, (_id, new_path))
